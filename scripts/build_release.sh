...
# Build the app
build_app() {
    print_status "Building DevIconGenerator..."
    
    # Build without signing for local development
    xcodebuild -project DevIconGenerator.xcodeproj \
               -scheme DevIconGenerator \
...
create_dmg() {
    ...
    mkdir -p "$DMG_DIR"
      ...
      cp -R "$EXPORT_DIR/DevIconGenerator.app" "$DMG_DIR/"
    ...
    DMG_NAME="DevIconGenerator-${VERSION}.dmg"
    
    print_status "Creating DMG: $DMG_NAME"
    
    create-dmg \
        --volname "DevIconGenerator" \
        --window-pos 200 120 \
        --window-size 600 360 \
        --icon-size 72 \
        --icon "DevIconGenerator.app" 165 180 \
        --icon "Applications" 435 180 \
        --hide-extension "DevIconGenerator.app" \
        --text-size 14 \
        --format ULFO \
        --filesystem HFS+ \
        --skip-jenkins \
...
show_usage() {
    ...
    echo ""
    echo "This script builds DevIconGenerator and creates a DMG for distribution."
}
...
# Main execution
main() {
    print_status "Starting DevIconGenerator release build..."
